#!/bin/sh
#
# install-vivaldi.sh Version 1.8.666.5
#
## Basic Usage ##
#
...
while [ 0 ]; do
  if [ "$1" = "-h" -o "$1" = "--help" -o "$1" = "help" ]; then
    ...
    if echo "$2" | grep -xq '\([0-9]\+\.\)\{3\}[0-9]\+-[0-9]\+'; then
      ...
      VIVALDI_VERSION="${2}-1" # 9 times of of 10 the packaging number is -1
    else
...
# Update the icon and desktop databases (non-essential)
updatedbs () {
...
# Remove Widevine and Proprietary media on uninstall
if [ "x\${1:-}" = "x--upgrade" ]; then :
else
  if [ -x "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-ffmpeg" ]; then
    if [ "\${USER:-}" = "root" ]; then
      "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-ffmpeg" --undo
      "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-widevine" --undo
    else
      "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-ffmpeg" --user --undo
      "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-widevine" --user --undo
    fi
  fi
fi
...
# Create a symlink between the old and new uninstall locations.
ln -fs "$UNINSTALL_VIVALDI_SCRIPT" ".$UNINSTALL_VIVALDI_SCRIPT_OLD"

# Create symlinks in /usr/local/bin or $HOME/bin when possible
...
if [ -x "$UNINSTALL_VIVALDI_SCRIPT" ]; then
  remove_previous_upgrade "$UNINSTALL_VIVALDI_SCRIPT" --upgrade
elif [ -x "$UNINSTALL_VIVALDI_SCRIPT_OLD" ]; then
...
# Avoids changing system directory permissions and ownership
...
# Run the widevine and proprietary media scripts and correct sandbox permissions
if [ "${USER:-}" = "root" ]; then
  chmod 4755 "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/vivaldi-sandbox"
  if [ -x "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-ffmpeg" ]; then
    "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-widevine" >/dev/null 2>&1 ||:
    "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-ffmpeg" >/dev/null 2>&1 ||:
  fi
else
  if [ -x "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-ffmpeg" ]; then
    "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-widevine" --user >/dev/null 2>&1 ||:
    "$VIVALDI_INSTALL_DIR/$VIVALDI_STREAM_SHORT/update-ffmpeg" --user >/dev/null 2>&1 ||:
  fi
fi
